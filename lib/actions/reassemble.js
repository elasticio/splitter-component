...
async function timer(this_) {
  for (let i = 0; i < groupList.length; i += 1) {
    ...
    // check if messageId defined
    if (Object.keys(results.messageIdsSeen).length !== 0) {
      const incomingData = {};
      ...
      const incomingData = [];
      results.messages.forEach((message) => {
        incomingData.push(message.messageData);
      });
      // eslint-disable-next-line no-await-in-loop
      await this_.emit('data', messages.newMessageWithBody({
        groupSize: results.messages.length,
...
async function processAction(msg, cfg) {
  ...
  if (messageId) {
    ...
    messagesNumberSeen = Object.keys(parsedMessageGroup.messageIdsSeen).length;
  } else {
    messagesNumberSeen = Object.keys(parsedMessageGroup.messages).length;
  }
  ...
  if (mode === 'groupSize') {
    if (messagesNumberSeen >= messageGroupSize) {
      ...
      } else { // If no messageID print all messageData in an Array
        const incomingData = [];
        await parsedMessageGroup.messages.forEach((message) => {
          incomingData.push(message.messageData);
        });
        await this.emit('data', messages.newMessageWithBody({
          groupSize,
