...
async function processAction(msg, cfg) {
  ...
  if (timersec && (isNumberNaN(timersec) || timersec <= 0)) {
    throw new Error('"Time the process waits when no incoming messages before emitting" must be a positive integer.');
  }
  if (groupSize) {
...
async function getMetaModel(cfg) {
  ...
  if (mode === 'timeout' || mode === 'groupSize&timeout') {
    result.in.properties.timersec = {
      type: 'number',
      help: {
        description: 'Time the process waits when no incoming messages before emitting (Default 20000 milliseconds)',
      },
